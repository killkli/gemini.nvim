...
M.setup = function()
	local register_menu = function(name, command_name, menu, system_prompt)
		local gemini = function(opts)
			M.show_stream_response(name, system_prompt, opts)
		end
...
M.prepare_code_prompt = function(prompt, bufnr, start_line, end_line)
	...
	if start_line and end_line then
		...
		-- Use the entire buffer if not in visual mode
...
M.show_stream_response = function(name, system_prompt, opts)
	...
	local prompt = M.prepare_code_prompt(system_prompt, current, start_line, end_line)
